...
users.num=2; 
dis_rt=[20,30]; % distance between the transmitter and receiver
flagUsers='NF-NF'; % Both users are located in "NF-NF" or "FF-FF" or "NF-FF"
%% Simulation setting
...
%% Theoretical performance
    [TheoChannel_Ac,TheoChannel_FF,TheoChannel_FF_ap]=MUChApproximate(par,temp_UETX_dis,termConstant,flagLoS,flagNLoS,users);
...
switch flagUsers
    case 'NF-NF'
        ...
        legend('Practical (NF-NF)','Theo. (NF-NF)', 'Interpreter','latex')
    case 'FF-FF'
    ...
    % h(1) = plot(NaN,NaN,'-','Color',[0.91,0.47,0.47],'linewidth',2); 
    % h(2) = plot(NaN,NaN,'-','Color',[0.45,0.43,0.86],'linewidth',2); 
    legend('Practical (FF-FF)','Theo. (FF-FF)', 'Interpreter','latex')
    case 'NF-FF'
    ...
    %  h(1) = plot(NaN,NaN,'-','Color',[0.91,0.47,0.47],'linewidth',2); 
    % h(2) = plot(NaN,NaN,'-','Color',[0.29,0.64,0.79],'linewidth',2);
    legend('Practical (NF-FF)','Theo. (NF-FF)', 'Interpreter','latex')
    otherwise
...
title([num2str([flagLoS,flagNLoS]),', Capacity for ',num2str(tra.totalNum),'$\times$', num2str(rec.totalNum),' array',', $d_{rt}=$', num2str(abs(dis_rt)) ],'Interpreter','latex')
grid on

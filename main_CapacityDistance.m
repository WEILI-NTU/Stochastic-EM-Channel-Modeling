...
clear   
%%========================================================  
dis_rt_Range=[5:100:1500]; % distance between the transmitter and receiver
...
RayleighDis_rec=0.62*sqrt((0.5*par.wav)^3/par.wav)/par.wav;
RayleighDis_tra=0.62*sqrt((tra.num_x*tra.spac)^3/par.wav)/par.wav;
RayleighDis=sqrt(RayleighDis_rec*RayleighDis_tra);

RayleighDis_tra=2*((tra.num_x*tra.spac)^2)/(par.wav^2);
RayleighDis_rec=2*((0.5*par.wav)^2)/(par.wav^2);
sqrt(RayleighDis_rec*RayleighDis_tra)
...
figure
hold on
plot(dis_rt_Range,Capa_XX_iso(:,snrVal),'-','color',[0.8500 0.3250 0.0980],...
    'linewidth',2,'MarkerSize',1)
plot(dis_rt_Range,Capa_DP_iso(:,snrVal),'-o','color',[0.93,0.69,0.13],...
    'linewidth',2,'MarkerSize',1)
plot(dis_rt_Range,Capa_TP_iso(:,snrVal),'-o','Color',[0.13,0.62,0.77]...
    ,'linewidth',2,'MarkerSize',1)
xlabel('distance ($\lambda$) ','Interpreter','latex');
ylabel('Capacity (bits/s/Hz)','Interpreter','latex');
legend('XX (Ideal)','DP (Ideal)','TP (Ideal)',...  
    'Interpreter','latex');
grid on 

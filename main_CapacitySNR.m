...
clear   
%%========================================================  
dis_rt_Range=100; % distance between the transmitter and receiver
...
for snrVal=1:length(varNoise)
    ...
    for MC=1:MC_max
        ...
        IsoChOper= cellfun(@(x,y)  termConstant*flagLoS*x+ flagNLoS*y,CoGreenOper,NLOSOper ,'UniformOutput',false); 
        ...
        CoupleChOper= cellfun(@(z)z*CoupleMatrix*diag(MatrixTotalEffic),IsoChOper,'UniformOutput',false);
...
figure
hold on
plot(SNR_range,Capa_XX_iso(iter_DisRT,:),'-o','color',[0.8500 0.3250 0.0980],...
    'linewidth',2,'MarkerSize',4)
plot(SNR_range,Capa_DP_iso(iter_DisRT,:),'*-','color',[0.93,0.69,0.13],...
    'linewidth',2,'MarkerSize',4)
plot(SNR_range,Capa_TP_iso(iter_DisRT,:),'-d','Color',[0.13,0.62,0.77]...
    ,'linewidth',2,'MarkerSize',4)
plot(SNR_range,Capa_XX_Cou(iter_DisRT,:),':o','color',[0.8500 0.3250 0.0980],...
    'linewidth',2,'MarkerSize',4)
plot(SNR_range,Capa_DP_Cou(iter_DisRT,:),':*','color',[0.93,0.69,0.13],...
    'linewidth',2,'MarkerSize',4)
plot(SNR_range,Capa_TP_Cou(iter_DisRT,:),':d','Color',[0.13,0.62,0.77]...
    ,'linewidth',2,'MarkerSize',4)
plot(SNR_range,Cap_Rayleigh(iter_DisRT,:),'-^','Color',[0.42,0.73,0.36],'linewidth',2,'MarkerSize',3)
 
xlabel('SNR (dB) ','Interpreter','latex');
ylabel('Capacity (bits/s/Hz)','Interpreter','latex');
legend('XX (w/o MC)','DP (w/o MC)','TP (w/o MC)',...
    'XX (with MC)','DP (with MC)','TP (with MC)','i.i.d. Rayleigh fading'...  
    ,  'Interpreter','latex');
grid on 
title([num2str([flagLoS flagNLoS]),' Capacity for ',num2str(tra.totalNum),'$\times$', num2str(rec.totalNum),' array, $d_{RT}=$', num2str(dis_rt_Range(iter_DisRT)),'$\lambda$'],'Interpreter','latex')

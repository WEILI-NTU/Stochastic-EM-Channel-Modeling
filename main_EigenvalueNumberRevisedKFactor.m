...
tra.num_x=20; % number of antennas in x-direction
tra.num_y=5; % number of antennas in y-direction 
% receive antenna array setting
rec.spac=0.1*par.wav;% spacing between adjacent antennas in array
rec.num_x=5; % number of antennas in x-direction
rec.num_y=3; % number of antennas in y-direction
...
for MC=1:MC_max
   ...
   NLOSOper= cellfun(@(x,y)  termConstant*termQV*x+ (termConstant*termQV)*sqrt(y).*randn(size(y,1),size(y,2)),IncoGreenExpOper,IncoGreenVarOper ,'UniformOutput',false);
    ...
    UnCoupCapa=cellfun(@(x)  x'*x,  IsoChOper_temp, 'UniformOutput',false); 
...
grid on 
title(['$K=$', num2str(KFactor),', $N_s$=',num2str(tra.totalNum),', $N_r$=',num2str(rec.totalNum), ...
    ', $\Delta_s$=',num2str(tra.spac/par.wav),'$\lambda$', ...
    ', $\Delta_r$=',num2str(rec.spac/par.wav),'$\lambda$'],'Interpreter','latex')
 ylim([-20 150]);
 
xlim([0 rec.totalNum])
% pi*(rec.num_x-1)*rec.spac*(rec.num_y-1)*rec.spac/(par.wav^2)
fprintf('end')

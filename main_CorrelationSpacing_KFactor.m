...
clear   
%%======================================================== 
dis_rt=15; % distance between the transmitter and receiver
KFactor=10; % K factor for Rician channel 
MC_max=30000; % Monto-Carlo
%%  Set the parameters
...
tra.spac=0.005*par.wav;% spacing between adjacent antennas in array
num_Sampling=200;
...
for MC=1:MC_max
    ...
    tempIsoDP=tempIsoDP+(UnCoupCapa{1}(1,:)/max(UnCoupCapa{1}(1,:)) ...
     ...
     %% Rician channel
    LoSChannel= cellfun(@(x)  termConstant*x,struct2cell(CoGreen),'UniformOutput',false);
    
     K_constant_Ric= cellfun(@(x)   (sum(norm(termConstant)^2*x.*conj(x))/length(x) ) ...
            ,CoGreenOper,'UniformOutput',false); 
flagNLoS_Ric= cellfun(@(x) sqrt(x/(KFactor+x)) ,K_constant_Ric,'UniformOutput',false); 
flagLoS_Ric= cellfun(@(x) sqrt(1-x^2) ,flagNLoS_Ric,'UniformOutput',false);  
    ...
    h_rayleigh=h_rayleigh./(par.UETX.distance.^2);
    % To generate a Rician fading channel gain
    % h_rician = real(flagLoS_Ric{1} *LoSChannel{1}+flagLoS_Ric{2} *LoSChannel{2})/2 + flagNLoS_Ric{1} * h_rayleigh;   
    h_rician = flagLoS_Ric{2} *  (LoSChannel{2}) +  flagNLoS_Ric{2} * h_rayleigh; 
...
plot(XRange,Cor_Rician,'-','Color',[0.44,0.39,0.86],'linewidth',2) 

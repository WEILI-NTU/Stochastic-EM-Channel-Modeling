...
K_constant= cellfun(@(x,y,z)   (sum(norm(termConstant)^2*x.*conj(x))/length(x)) ...
    /(sum(norm(termConstant*termQV)^2*y.*conj(y))/length(y) ... 
...
for snrVal=1:length(varNoise)
    ...
    for MC=1:MC_max
        ...
        % 
        % Corician=real(flagLoS{1}^2*SumLoSChanOper{1}+ ...
        %     flagNLoS{1}^2*exp(1j*2*pi*dis_rt_Range(iter_DisRT)*par.wav/20));
        % lambda_RicianTemp=sum(log2(1+(eig(Corician/varNoise(snrVal) ) ) ) );
        % Cap_Rician_sum=Cap_Rician_sum+sum(lambda_RicianTemp);
        ...
        h_rician = flagLoS{1} * (LoSChannel{1}) + flagNLoS{1} * h_rayleigh; 
